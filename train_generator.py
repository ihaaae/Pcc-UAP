...
@torch.no_grad()
def test_wav(model:UAP, filename_list, data_folder, out_folder, speaker_model=None, label_dict=None, target=-1, noise_scale=1):
    model.eval()
    ...
    for idx, filename in enumerate(bar):
        ...
        pout = model().squeeze().detach().cpu().numpy()
...
def test_interpolation(model:UAP, filename_list, data_folder, out_folder, speaker_model=None, label_dict=None, target=-1, noise_scale=1, beta=0):
    model.eval()
    if speaker_model: speaker_model.eval()
    noise_dim = model.noise_dim
    batch_size = 1
...
def batch_process_generator(model:UAP, data, train_mode=True, **kwargs):
    wav_data, speaker_id, norm_factor = data
    wav_data, speaker_id = wav_data.float().cuda(), speaker_id.long().cuda()
...
def get_parser():
    ...
    parser.add_argument("--speaker_model", type=str, default="../SincNet_TIMIT/model_raw.pkl", help="path for pretrained speaker model")
...
def main(args):
    ...
    writer = SummaryWriter(args.output_dir + '/tb-logs')
    trainer = ClassifierTrainer(model, train_dataloader, optimizer, cost, batch_process, args.output_dir, 0, 
            ...
            batch_param={'noise_scale':args.noise_scale, "target":args.target}, writer=writer)
